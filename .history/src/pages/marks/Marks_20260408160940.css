...
function Marks() {
  ...
  const fetchSubjects = async (sem) => {
    ...
    const res = await api.get(`/subjects/by-semester?semester=${sem}`);
  ...
  const fetchStudents = async () => {
    ...
    const res = await api.get(
      `/students/filter-sem-section?semester=${semester}&section=${section}`
    );
  ...
  const saveMarks = async () => {
    ...
    if (!subject || !semester || !testType || !testNo) {
