  ...
  const calculateAnalysis = (groupedData, subjectsList) => {
    ...
    const subjectStats = subjectsList.map((sub) => {
      ...
      return {
        ...
        pass,
        fail,
        passPercent: appeared === 0 ? 0 : ((pass / appeared) * 100).toFixed(2),
      };
    });

    /* 🔥 FAIL DISTRIBUTION + NIL */
    ...
    students.forEach((s) => {
      ...
      subjectsList.forEach((sub) => {
        ...
        if (mark === "AB") {
          hasAbsent = true; // ❌ absent = not all clear
        } else if (Number(mark) < 50) {
          failCount++;
        }
      });

      /* ✅ NIL = NO FAIL + NO ABSENT */
      if (failCount === 0 && !hasAbsent) {
        allCleared++;
      } else {
        const key = failCount || "AB"; // optional grouping
        failDistribution[failCount] = (failDistribution[failCount] || 0) + 1;
      }
    });
      ...
      <div className="result-page">
        ...
        <div id="pdf-content" className="pdf-container">
          <div className="college-block">
            ...
            <h4>UNIVERSITY EXAM RESULT REPORT</h4>
          </div>
